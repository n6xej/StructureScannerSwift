        ...
        _enhancedColorizeTask = try! STColorizer.newColorizeTaskWithMesh(mesh, scene: _slamState.scene, keyframes: _slamState.keyFrameManager!.getKeyFrames(), completionHandler: {error in
            ...
            else
            {
                dispatch_async(dispatch_get_main_queue(), {
                    ...
                    //FIXME: the enhanced mesh crashes if uploaded
                    // in MeshRenderer:uploadTexture
                    self.meshViewController.highmesh = mesh
                    // self.meshViewController.mesh = mesh
                })
                ...
                // when using STColorizerType.TextureMapForObject it crashes on upload mesh
                // uploadTexture(mesh.meshYCbCrTexture as! CVPixelBufferRef)  PerVertex
            
        // end of STColorizer.newColorizeTaskWithMesh
        )
